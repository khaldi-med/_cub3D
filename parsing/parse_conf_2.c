...
/*   Updated: 2026/01/06 02:43:58 by mohkhald         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_read_line(int fd, t_config *config, int *row, int map_l)
{
	char	*line;
	char	*new_line;
...
void	ft_parse_file(char *arr, t_config *config)
{
	...
	row = 0;
	ft_read_line(fd, config, &row, map_l);
	close(fd);
}
